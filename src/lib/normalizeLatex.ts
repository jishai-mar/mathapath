 ...
 */
/**
 * Converts a system of equations into proper LaTeX cases environment
 * Detects patterns like "x + y = 10 x - y = 2" or equations separated by newlines
 */
export function convertSystemOfEquations(input: string): string {
  ...
  // Already has cases environment
  if (input.includes('\\begin{cases}') || input.includes('\\begin{array}')) {
    return input;
  }
