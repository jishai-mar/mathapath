...
import { parseAndValidate, generateExerciseSchema } from "../_shared/validation.ts";
...
serve(async (req) => {
  ...
  try {
    // Validate input
    const validation = await parseAndValidate(req, generateExerciseSchema, corsHeaders);
    if (!validation.success) {
      return validation.response;
    }
    const { subtopicId, difficulty, existingExercises, userId, performanceData, subLevel } = validation.data;
