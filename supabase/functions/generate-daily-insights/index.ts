...
serve(async (req) => {
  ...
  try {
    ...
    const progressSummary = topicProgress?.map((t: any) => 
      `${t.name}: ${t.mastery}% mastery`
    ).join(', ') || 'No progress data yet';

    const weakAreas = weakSubtopics?.map((s: any) => s.subtopic_name).join(', ') || 'None identified';

    const personalityPrompts: Record<string, string> = {
      patient: 'Be gentle, supportive, and break down recommendations into manageable steps.',
      encouraging: 'Be enthusiastic, celebratory, and focus on achievements while motivating improvement.',
      challenging: 'Be direct, push for excellence, and set ambitious but achievable goals.',
      humorous: 'Be light-hearted, use friendly humor, and make learning feel fun.'
    };

    const personalityStyle = personalityPrompts[tutorPersonality] || personalityPrompts.patient;
...
Generate personalized daily insights for this student.`;
    ...
    try {
      ...
      insights = {
        ...
        insights: [
          { type: "tip", text: "Consistent practice is key to mastering mathematics." }
        ],
